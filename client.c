...
static const struct opt_table opts[] = {
	...
	OPT_WITH_ARG("--devmem-src-dev <arg>", opt_set_charp, opt_show_charp,
		     &opt.devmem_dst_dev, "Select the source device for the TCP Devmem memory provider"),
	OPT_WITH_ARG("--devmem-dst-dev <arg>", opt_set_charp, opt_show_charp,
		     &opt.devmem_src_dev, "Select the destination device for the TCP Devmem memory provider"),
	OPT_ENDTABLE
};
